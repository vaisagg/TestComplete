...
# Copy the IoT files
try {
   Write-Host "Copying MechanismEmulationConfig from $contentPath to $destPath on the IoT Device..."
   Copy-Item -Path $contentPath -Destination $destPath -Recurse -Force -ToSession $IoTBoard
}
catch {
   Write-Error "Failed to copy MechanismEmulationConfig to the IoT Device."
   Return 1
}
