...
param(
  ...
  [string] $contentPath = "C:\Siemens\ARM\Bin\Configuration\Overwrite for Test Fixture\MechanismEmulationConfig.txt",
...
try {
   ...
   Invoke-Command $IoTBoard -ScriptBlock {rm C:\data\startIotlogs.txt} -EA SilentlyContinue
...
try {
   ...
   Write-Host "Starting DML process.."
    Invoke-Command $IoTBoard -ScriptBlock {& 'C:\data\deploy\StartDML_IoT.exe' | Tee-Object "C:\data\deploy\startIot.txt"}
