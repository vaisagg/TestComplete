...
param(
  [string] $contentPath = "C:\Siemens\ARM\Emulator\SupplySensorStates.txt", #Ramitha modified the path#
  [string] $destPath= "C:\Data\Deploy\SupplySensorStates.txt",
  [string] $destPath1= "C:\Data\SupplySensorStates.txt",   #samir added#
...
Invoke-Command $IotBoard -ScriptBlock {rm C:\Data\SupplySensorStates.txt} -EA SilentlyContinue     #samir added#
...
try {
   ...
   Invoke-Command $IoTBoard -ScriptBlock {Move-Item -Path C:\Data\SupplySensorStates.txt -Destination C:\Data\Deploy\SupplySensorStates.txt -Force}   #samir added#
