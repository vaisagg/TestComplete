...
param(
  #[string] $contentPath = "C:\Siemens\ARM\Emulator\SensorStateChange.txt", #Ramitha modified the path#----------commneted by shrutika-12-Nov-2019
  [string] $contentPath = "C:\data\deploy\simulation\SensorStateChange.txt",
  [string] $destPath= "C:\Data\Deploy\SensorStateChange.txt",
  [string] $destPath1= "C:\Data\SensorStateChange.txt",   #samir added#
...
Invoke-Command $IotBoard -ScriptBlock {rm C:\Data\SensorStateChange.txt} -EA SilentlyContinue     #samir added#
...
try {
   ...
   Invoke-Command $IoTBoard -ScriptBlock {Move-Item -Path C:\Data\SensorStateChange.txt -Destination C:\Data\Deploy\SensorStateChange.txt -Force}   #samir added#
